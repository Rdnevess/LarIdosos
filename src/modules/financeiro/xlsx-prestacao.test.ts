...
import type { DocumentoPrestacao, LinhaDespesa } from './documento-prestacao'
import { gerarXlsxPrestacao } from './xlsx-prestacao'

/**
 * O renderizador não toca o banco: recebe um `DocumentoPrestacao` pronto. O
 * documento de teste é montado aqui, à mão, para o teste falhar por causa do
 * `.xlsx` e nunca por causa de uma consulta.
 */
function documentoDeTeste(sobrescritas: Partial<DocumentoPrestacao> = {}): DocumentoPrestacao {
  return {
    capa: {
...
function despesaDeTeste(sobrescritas: Partial<LinhaDespesa> = {}): LinhaDespesa {
  return {
    item: 1,
    credor: 'Energisa',
...
describe('gerarXlsxPrestacao', () => {
  ...
  it('grava valores calculados, e nenhuma fórmula', async () => {
    ...
    const documento = documentoDeTeste({
      ...
      receitas: [
        { item: 1, origem: 'Doação', documento: '', data: new Date(2026, 7, 5), valor: 2000 },
      ],
    })
