...
function anotacaoDe(editavelAte: Date, criadoPorId: string | null) {
  return { id: 'ant_1', editavelAte, criadoPorId }
}

describe('exigirJanelaAberta', () => {
  it('deixa o autor corrigir dentro do prazo', () => {
    expect(() => exigirJanelaAberta(anotacaoDe(prazoDeEdicao(), 'usr_1'), ctx, 'Anotacao')).not.toThrow()
  })
  ...
  it('recusa depois do prazo, mandando retificar', () => {
    ...
    expect(() => exigirJanelaAberta(anotacaoDe(expirado, 'usr_1'), ctx, 'Anotacao')).toThrow(ErroValidacao)
  })

  it('recusa quem não escreveu, mesmo dentro do prazo', () => {
    expect(() => exigirJanelaAberta(anotacaoDe(prazoDeEdicao(), 'usr_2'), ctx, 'Anotacao')).toThrow(ErroPermissao)
  })
  ...
  it('recusa quando a anotação não tem autor registrado', () => {
    ...
    expect(() => exigirJanelaAberta(anotacaoDe(prazoDeEdicao(), null), ctx, 'Anotacao')).toThrow(ErroPermissao)
  })
  ...
  it('confere a autoria antes do prazo', () => {
    ...
    const expirado = new Date(Date.now() - 60_000)
    expect(() => exigirJanelaAberta(anotacaoDe(expirado, 'usr_2'), ctx, 'Anotacao')).toThrow(ErroPermissao)
  })
